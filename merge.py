...
def mergexml(xmlfile_list):
    if len(xmlfile_list) == 1:
        xmlfilename = xmlfile_list[0]
        assfilename = xmlfile_list[0][:xmlfile_list[0].rindex('.')] + '.ass'
        cmd = ['DanmakuFactory', '-i', xmlfilename, '-o', assfilename]
        print(cmd)
    ...
    assfilename = xmlfile_list[0][:xmlfile_list[0].rindex('.')] + '-merge.ass'
    cmd = ['DanmakuFactory', '-i', xmlfilename, '-o', assfilename]
    print(cmd)
    ps = subprocess.Popen(cmd, shell=False)
    ps.wait()
